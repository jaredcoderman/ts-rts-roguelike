...
export class GameManager {
	...
	public static getInstance(canvas?: HTMLCanvasElement): GameManager {
		if (!GameManager.instance) {
			if (!canvas) {
				throw new Error(
					'Canvas must be provided upon GameManager instance creation'
				)
			}
	...
	public async init() {
		...
		let r = new Resource(400, 500, 20, 'Tiles/resource.png', 20, 15)
		let r2 = new Resource(400, 520, 20, 'Tiles/resource.png', 20, 15)
		let r3 = new Resource(420, 500, 20, 'Tiles/resource.png', 20, 15)
	...
	findClosestOfType<T extends GameObject>(
		...
		const objectsOfType = this.gameObjects.filter(
	...
	queryResources(point: Vector2): Resource | undefined {
		const resources = this.gameObjects.filter(
			(gameObject) => gameObject instanceof Resource
		)
		let resourceOnPoint = resources.find(
			(resource) => resource.containsPoint(point) && resource.available
		)
	...
	findResource(collector: Collector): Resource | undefined {
		...
		const availableResources = resources.filter(
			(resource) => resource.available && !resource.beingExtracted
		)
		let closestDistance = Infinity
